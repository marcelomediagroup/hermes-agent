...
| **Free-response channels** | You can make specific channels mention-free with `DISCORD_FREE_RESPONSE_CHANNELS`, or disable mentions globally with `DISCORD_REQUIRE_MENTION=false`. Messages in these channels are answered inline by default so the channel stays a lightweight chat. If you want a mention-free intake lane that still spins each request into its own thread, add it to `DISCORD_THREADED_FREE_RESPONSE_CHANNELS`. |
...
discord:
  ...
  allow_mentions:                 # What the bot is allowed to ping (safe defaults)
    ...
    replied_user: true            # reply-reference pings the author (defaults false in final-mention mode)
...
You usually pair this with `free_response_channels` for the same channel. The channel then behaves like:
