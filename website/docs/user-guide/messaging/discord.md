...
To use mention-only Discord notifications, set `mention_user_on_final: true`. Hermes then starts the turn-final response with the requesting user's explicit `<@user_id>` mention while leaving tool progress and interim messages unmentioned. Proactive messages without a requesting user are unchanged. Keep `allow_mentions.users: true` so Discord delivers the ping. Final-mention mode also defaults `allow_mentions.replied_user` to `false`, preventing ordinary reply references from generating early notifications; setting it explicitly to `true` overrides that safeguard.
